...
MONOPIC_ADDRESS = "https://maps.app.goo.gl/uhPe2RrzLEAkUpH88"
...
Terima kasih telah melakukan reservasi di Monopic Studio.
...
def render_message_template(message_template: str, booking: dict, paket: dict, user: dict) -> str: # establish database connection
    ...
    rendered_message = rendered_message.replace("{{tanggal}}", booking['scheduled_at'].strftime("%d-%m-%Y"))
    rendered_message = rendered_message.replace("{{waktu}}", f"{booking['scheduled_at'].strftime('%H:%M')}")
...
def send_whatsapp():
    ...
    try:
        ...
        phone_number = normalize_phone(user["hp"])
